...
void ReportTimings(FILE * timingfile) {
	...
	fprintf(timingfile, "---> %6.2f MB/sec on %6.3f GB", total_disk_read/(thistime+1e-15)/1e6, total_disk_read/1e9);
    REPORT(0, "Total Disk Blocking Writes", TotalWrite.Elapsed());
	fprintf(timingfile, "---> %6.2f MB/sec on %6.3f GB", total_disk_write/(thistime+1e-15)/1e6, total_disk_write/1e9);
