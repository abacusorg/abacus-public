    ...
    if(Output){
        ...
        denom = thistime;
            ...
            REPORT(2, "Output Light Cone", OutputLightCone.Elapsed());
                REPORT_RATE(Output);
            denom = thistime;
            REPORT(3, "Setup", LCSetup.Elapsed());
                REPORT_RATE(Output);
            REPORT(3, "Search", LCSearch.Elapsed());
                REPORT_RATE(Output);
            REPORT(3, "Gather Output", LCGatherOutput.Elapsed());
                REPORT_RATE(Output);
            REPORT(4, "Create Sparse Map", LCCreateSparseMap.Elapsed());
            REPORT(5, "Sort Healpix", LCSortHealpix.Elapsed());
                fmt::print(reportfp,"---> {:6.2f} Mint/sec",thistime ? ((OutputDep *) Output)->np_lightcone/thistime/1e6 : 0.);
            REPORT(3, "Free SlabAccum", LCFreeSlabAccum.Elapsed());
