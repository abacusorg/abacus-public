...
void FinalizeWriteState() {
    WriteNodeSlabs();  // We do this here because it will need a MPI Barrier
        ...
        MPI_Allreduce(MPI_IN_PLACE, &WriteState.MaxAcceleration, 1, MPI_DOUBLE, MPI_MAX,  MPI_COMM_WORLD);
        // Maximize MaxVelocity
        MPI_Allreduce(MPI_IN_PLACE, &WriteState.MaxVelocity, 1, MPI_DOUBLE, MPI_MAX,  MPI_COMM_WORLD);
        // Minimize MinVrmsOnAmax
        MPI_Allreduce(MPI_IN_PLACE, &WriteState.MinVrmsOnAmax, 1, MPI_DOUBLE, MPI_MIN,  MPI_COMM_WORLD);
        // Minimize MinCellSize
        MPI_Allreduce(MPI_IN_PLACE, &WriteState.MinCellSize, 1, MPI_INT, MPI_MIN,  MPI_COMM_WORLD);
        // Maximize MaxCellSize
        MPI_Allreduce(MPI_IN_PLACE, &WriteState.MaxCellSize, 1, MPI_INT, MPI_MIN,  MPI_COMM_WORLD);
        // sqrt(Sum(SQR of RMS_Velocity))
        MPI_Allreduce(MPI_IN_PLACE, &WriteState.RMS_Velocity, 1, MPI_DOUBLE, MPI_SUM,  MPI_COMM_WORLD);
        // sqrt(Sum(SQR of StdDevCellSize))
        MPI_Allreduce(MPI_IN_PLACE, &WriteState.StdDevCellSize, 1, MPI_DOUBLE, MPI_SUM,  MPI_COMM_WORLD);
