                ...
                for (int a=0; a<FOFlevel1[g].ngroups; a++) {
                    ...
                    for (int b=0; b<size; b++) {
                        ...
                        if (groupacc != NULL){
                            ...
                            L1vel[g][b] -= (acc3struct)(L1acc[g][b])*WriteState.FirstHalfEtaKick;
                        }
                    }
                    GFC->L2FOF.Start();
                    ...
                    for (int b=0; b<size; b++)
                        if (groupaux[start[b].index()].is_taggable()) {
                            ...
                            // Velocities were full kicked; half-unkick before halostats
                            if (groupacc != NULL)
                                v -= (acc3struct)(groupacc[start[b].index()])*WriteState.FirstHalfEtaKick;
            ...
            for (int n=0; n<globalgroups[j][k].size(); n++) {
                ...
                for (int c=0; c<globalgroups[j][k][n].ncellgroups; c++, cglink++) {
                    ...
                    for(int pi = start; pi < start + cg->size(); pi++){
                        ...
                        velstruct _v = vel[pi] - unkick_factor*(acc3struct)(acc[pi]);
