...
from multipoles_metacode_utils import (
    emit_dispatch_function,
    Writer,
    default_metacode_argparser,
    cmapper,
)
...
def emit_AVX512_Multipoles(orders, fn='CMAVX512.cpp'):
    ...
    for order in orders:
        ...
        for a in range(order + 1):
            ...
            for b in range(order - a + 1):
                ...
                for c in range(order - a - b + 1):
                    w(
                        f'CM[{i}] = AVX512_MASK_ADD_DOUBLES(CM[{i}], left_mask, CM[{i}], fijk);'
                    )
                    ops += 1
                    i += 1
                    if c < order - a - b:
    ...
    emit_dispatch_function(
        w,
        """Multipole512Kernel(FLOAT3 *xyz, int n, FLOAT3 center, double *CM)""",
        orders,
    )
...
def emit_AVX512_Multipoles_FMA(
    orders, fn='CMAVX512.cpp', max_zk=None, unroll=4, naccum=1
):
    ...
    for order in orders:
        ...
        def _emit_unrolled(start, unroll, mask=False):
            ...
            for c in range(1, this_max_zk):
                for u in range(unroll):
                    w(
                        f'zk{c}[{u}] = AVX512_MULTIPLY_DOUBLES(zk{c-1}[{u}], deltaz[{u}]);'
                    )
                ops += 1
                ...
                for b in range(order - a + 1):
                    for c in range(order - a - b + 1):
                        if c % this_max_zk == 0 and c > 0:
                            for u in range(unroll):
                                w(
                                    f'fij[{u}] = AVX512_MULTIPLY_DOUBLES(fij[{u}], zk{this_max_zk-1}[{u}]);'
                                )
                            ops += 1
                        for u in range(unroll):
                            w(
                                f'CM[{i}][{u%naccum}] = AVX512_FMA_ADD_DOUBLES(fij[{u}], zk{c % this_max_zk}[{u}], CM[{i}][{u%naccum}]);'
                            )
                        ops += 2
                        i += 1
                    if b < order - a:
...
def emit_AVX512_Taylors(orders, fn='ETAVX512.cpp'):
    ...
    emit_dispatch_function(
        w,
        'Taylor512Kernel(double *CT, FLOAT3 center, int n, FLOAT3 *xyz, FLOAT3 *acc)',
        orders,
    )
