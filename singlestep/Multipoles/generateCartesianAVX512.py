...
def emit_AVX512_Multipoles(maxorder=16, fn='CMAVX512.cpp'):
	with open(fn, 'wb') as fp:
		...
		for order in xrange(maxorder+1):
			...
			i = 0
			for a in xrange(order+1):
				w('        fij = fi;')
				for b in xrange(order-a+1):
					w('        fijk = fij;')
					for c in xrange(order-a-b+1):
...
def emit_AVX512_Taylors(maxorder=16, fn='ETAVX512.cpp'):
	with open(fn, 'wb') as fp:
		...
		for order in xrange(maxorder+1):
			...
			i = 0
			w('    int i = 0;')
			for a in xrange(order):
				w('        fij = fi;')
				for b in xrange(order-a):
					w('        fijk = fij;')
					for c in xrange(order-a-b):
