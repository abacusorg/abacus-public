...
void SinkPencilPlan::copy_into_pinned_memory(List3<FLOAT> &pinpos, int start, int total, FLOAT *SinkPosSlab, int NearFieldRadius, uint64 Nslab) {
    ...
    memcpy(pinpos.X+start,    SinkPosSlab+cell[0].start,        sizeof(FLOAT)*N1);
    memcpy(pinpos.X+start+N1, SinkPosSlab+cell[wrapcell].start, sizeof(FLOAT)*N2);

    memcpy(pinpos.Y+start,    SinkPosSlab+Nslab+cell[0].start,        sizeof(FLOAT)*N1);
    memcpy(pinpos.Y+start+N1, SinkPosSlab+Nslab+cell[wrapcell].start, sizeof(FLOAT)*N2);
}
...
int SinkPencilPlan::load(int x, int y, int z, int NearFieldRadius) {
    ...
    for (int c=0; c<width; c++) {
        int zc = z + c - NearFieldRadius;
        
        //if( CP->WrapSlab(zc - node_z_start) < width &&
        //    CP->WrapSlab(node_z_start + node_z_size - zc) < width ){
            // Regular cell
        ...
        // Identify if we cross the wrap
        if(CP->WrapSlab(zc) == 0){
            assert(wrapcell == 0);
            wrapcell = c;  // might be c=0; that's fine
        }
    }
    return total;
}
...
void SinkPencilPlan::copy_from_pinned_memory(void *_pinacc, int start, 
    ...
    int cumulative_number = 0;
    int k = sinkindex%node_z_size;    // Reduce this to the Z index for this pencil
