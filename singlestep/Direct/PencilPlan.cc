...
void SinkPencilPlan::copy_into_pinned_memory(List3<FLOAT> &pinpos, int start, int total, FLOAT *SinkPosSlab, int NearFieldRadius, uint64 Nslab) {
    ...
    if constexpr (dim == PENCIL_DIM::Z || dim == PENCIL_DIM::XYZ) {
    ...
    if constexpr (dim == PENCIL_DIM::X || dim == PENCIL_DIM::Y || dim == PENCIL_DIM::XYZ) {
        ...
        if constexpr (dim == PENCIL_DIM::Y || dim == PENCIL_DIM::XYZ) {
...
void SinkPencilPlan::copy_from_pinned_memory(void *_pinacc, 
    int total, void *SinkAccSlab, int k, int NearFieldRadius) {
    ...
    accstruct *a = (accstruct *)SinkAccSlab+cell[0].start;
    // At the beginning of the row, we get to set the whole thing
    int nwritten = 0;
    if(k == 0){
        memcpy(a, pinacc, sizeof(accstruct)*total);
        nwritten += total;
    }
    else {
        int last_N;
        ...
        if (wrapcell > 0){
            ...
            a = (accstruct *) SinkAccSlab + cell[wrapcell].start;
            ...
            a = (accstruct *) SinkAccSlab + cell[2*NearFieldRadius].start;
            memcpy(a, pinacc, sizeof(accstruct)*last_N);
            nwritten += last_N;
        }
    }
...
void SourcePencilPlan::copy_into_pinned_memory(List3<FLOAT> &pinpos, int start, int total, FLOAT **SourcePosSlab, int NearFieldRadius, uint64 *Nslab) {
    ...
    for (int c=0; c<width; c++) {
        ...
        if constexpr (dim == PENCIL_DIM::Y || dim == PENCIL_DIM::XYZ) {
            FLOAT *p = SourcePosSlab[c] + cell[c].start + Nslab[c];
            FLOAT *d = pinpos.Y+start+cumulative_number;
            memcpy(d, p, sizeof(FLOAT)*N);
        }

        if constexpr (dim == PENCIL_DIM::Z || dim == PENCIL_DIM::XYZ) {
            FLOAT *p = SourcePosSlab[c] + cell[c].start + 2*Nslab[c];
...
// Export some instantiations
template void SinkPencilPlan::copy_into_pinned_memory<PENCIL_DIM::XYZ>(List3<FLOAT> &pinpos, int start, int total, FLOAT *SinkPosSlab, int NearFieldRadius, uint64 Nslab);
template void SinkPencilPlan::copy_into_pinned_memory<PENCIL_DIM::X>(List3<FLOAT> &pinpos, int start, int total, FLOAT *SinkPosSlab, int NearFieldRadius, uint64 Nslab);
template void SinkPencilPlan::copy_into_pinned_memory<PENCIL_DIM::Y>(List3<FLOAT> &pinpos, int start, int total, FLOAT *SinkPosSlab, int NearFieldRadius, uint64 Nslab);
template void SinkPencilPlan::copy_into_pinned_memory<PENCIL_DIM::Z>(List3<FLOAT> &pinpos, int start, int total, FLOAT *SinkPosSlab, int NearFieldRadius, uint64 Nslab);

template void SourcePencilPlan::copy_into_pinned_memory<PENCIL_DIM::XYZ>(List3<FLOAT> &pinpos, int start, int total, FLOAT **SourcePosSlab, int NearFieldRadius, uint64 *Nslab);
template void SourcePencilPlan::copy_into_pinned_memory<PENCIL_DIM::X>(List3<FLOAT> &pinpos, int start, int total, FLOAT **SourcePosSlab, int NearFieldRadius, uint64 *Nslab);
template void SourcePencilPlan::copy_into_pinned_memory<PENCIL_DIM::Y>(List3<FLOAT> &pinpos, int start, int total, FLOAT **SourcePosSlab, int NearFieldRadius, uint64 *Nslab);
template void SourcePencilPlan::copy_into_pinned_memory<PENCIL_DIM::Z>(List3<FLOAT> &pinpos, int start, int total, FLOAT **SourcePosSlab, int NearFieldRadius, uint64 *Nslab);
