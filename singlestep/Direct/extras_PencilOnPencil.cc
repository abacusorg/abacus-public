...
void SetInteractionCollection::CPUExecute(){
    ...
    for (int j=0; j<NSinkSets; j++) {
        SinkPlan[j].copy_into_pinned_memory(*SinkSetPositions, SinkSetStart[j], SinkSetCount[j], SinkPosSlab, nfradius);
    }
    FillSinks.Stop();
    ...
    for (int j=0; j<NSourceSets; j++) {
        SourcePlan[j].copy_into_pinned_memory(*SourceSetPositions, SourceSetStart[j], SourceSetCount[j], SourcePosSlab, nfradius);
    }
    FillSources.Stop();
	...
	for (int k=0; k<cpd; k++) {
	    int idx = j*Nk+k;
	    SinkPlan[idx].copy_from_pinned_memory((void *)SinkSetAccelerations, SinkSetStart[idx], SinkSetCount[idx], (void *)SinkAccSlab, j, nfradius);
	}
    }
