...
\param{NumBlock}{int} This is the number of blocks to break the FFT
...
\todo{Probably this should be ZD\_NumBlock?  It's the lone exception.}


\todo{We need a parameter to set the directory for the temporary files.}
...
by BoxSize.  For example, one might need to convert between Mpc$^{-1}$ and
$h$~Mpc$^{-1}$ units.  Default value is 1.0.
...
\todo{We're being inconsistent about hMpc choices.  Shouldn't we be getting that too?}
...
\param{InitialConditionsDirectory}{string}
...
velocity to km/s.
\todo{Should change name to VelZSpace\_to\_kms!}
...
\param{MaxVelocity}{float} The maximum velocity component
...
\param{MaxAcceleration}{float} The maximum acceleration component
...
\todo{I think I could apply the max velocities on a cell basis after
all.}
...
\todo{But we should pick the endian status of intel, for simplicity.}
...
\todo{Should provide single precision too.}

\todo{DJE didn't write the packing correctly.  I want the upper 8 bits
of the first number to go into the first byte.}
