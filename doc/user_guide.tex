...
allocations.  Default is to detect the total (not free) amount of RAM.

\param{OMP_NUM_THREADS}{int} Number of OpenMP threads.  The default, 0, does not modify the system value
(usually OMP_NUM_THREADS, or all threads).  Negative values use that many fewer than the max.
For example, use $-4$ to use all but 4 cores.
