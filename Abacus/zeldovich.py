...
from Abacus import GenParam, abacus, transpose_ic
...
def calc_sigma8(params, z='init'):
    ...
    sig8_at_zinit = sig8_today * calc_growth_ratio(params, z, 0.0)
...
def setup_zeldovich_params(params):
    ...
    if params.get('sigma_8'):
        if params.get('ZD_Pk_file_redshift'):
            raise ValueError(
                'Must specify one of sigma_8 and ZD_Pk_file_redshift in parameter file'
            )
        ...
        if 'ZD_Pk_sigma_ratio' in params:
            if not np.isclose(growth_ratio, params['ZD_Pk_sigma_ratio'], rtol=1e-5):
                raise ValueError(
                    f'ZD_Pk_sigma_ratio = {params["ZD_Pk_sigma_ratio"]} in parameter file does not match value {growth_ratio} computed from ZD_Pk_file_redshift = {params["ZD_Pk_file_redshift"]}'
                )
        ...
        if not params.get('ZD_Pk_sigma') and not params.get('ZD_Pk_sigma_ratio'):
            raise ValueError(
                'Must specify one of ZD_Pk_file_redshift, sigma_8, ZD_Pk_sigma, or ZD_Pk_sigma_ratio in parameter file'
            )

    # Regardless of the growth amplitude method, calculate f_cluster
    cosm_zinit = Abacus.Cosmology.from_params(params, z='init')
    f_cluster_from_smooth = 1 - params.get('Omega_Smooth', 0.0) / params['Omega_M']
    if 'ZD_f_cluster' in params:
        ...
        if not np.isclose(f_cluster_from_smooth, params['ZD_f_cluster'], rtol=1e-5):
            raise ValueError(
                f'ZD_f_cluster = {params["ZD_f_cluster"]} does not match 1 - Omega_Smooth/Omega_M = {f_cluster_from_smooth}'
            )
    else:
...
def ensure_2d(param, nthread=2):
    ...
    transpose_ic.transpose(icdir, param['NP'], param['ICFormat'], nthread=nthread)
...
def run(paramfn, allow_eigmodes_fn_override=False, no_parallel=True, white=False):
    ...
    if 'ZD_PLT_filename' in params and not path.isfile(params['ZD_PLT_filename']):
        ...
        if path.basename(params['ZD_PLT_filename']) != path.basename(eigmodes_path):
            print(
                'Eigenmodes file "{}" did not exist!'.format(params['ZD_PLT_filename'])
            )
...
def run_override_dirs(parfn, out_parent, new_parfn='abacus_ic_fixdir.par', **kwargs):
    ...
    for k in new_params:
        ...
        if type(p) is str and 'abacus/' in p:
            new_params[k] = pjoin(
                os.getenv('ABACUS'), p[p.find('abacus/') + len('abacus/') :]
            )
    ...
    if new_params['ZD_qPLT']:
        ...
        if not path.isfile(eigmodes_fn):
            ...
            if not path.isfile(eigmodes_fn):
                print(
                    'Warning: original eigenmodes filename "{}" not found.  Falling back to most current eigenmodes.'.format(
                        eigmodes_fn
                    )
                )
    ...
    # if
    if (
        'ZD_Pk_file_redshift' in kwargs or 'InitialRedshift' in kwargs
    ) and 'ZD_Pk_sigma_ratio' in new_params:
    ...
    new_params = GenParam.makeInput(
        new_parfn, parfn, del_keywords=del_keys, **new_params
    )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description=f'Run the zeldovich code (located in {zeldovich_dir})'
    )
    parser.add_argument(
        'parfile',
        nargs='+',
    ...
    parser.add_argument(
        '--out-parent',
        help='Overrides the parfile InitialConditionsDirectory (i.e. the zeldovich output directory) with PARENT/SimName/ic.'
        "  Create a new abacus_ic.par with the modified parameters: IC dir; and if don't exist: eigmodes, camb_matterpower",
        metavar='PARENT',
    )
    parser.add_argument(
        '--show-growth',
    ...
    parser.add_argument(
        '--white',
        help='Ignore power spectrum file and use ZD_Pk_powerlaw_index=0',
        action='store_true',
    )
        ...
        if show_growth:
            ...
            print(
                'sigma_8(z={zinit:g}) = {}\nsigma_8(z=0) = {}\nGrowth ratio D(z={zinit:g})/D(z=0) = {}\n'.format(
                    sigma8_zinit,
                    par['sigma_8'],
                    sigma8_zinit / par['sigma_8'],
                    zinit=par['InitialRedshift'],
                )
            )
        else:
