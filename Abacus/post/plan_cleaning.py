...
Original filename: extract_mergerhistory_slabwise_slim_mpi.py
'''
...
for i, ii in enumerate(range(nfiles)):
	...
	print("Cleanups...")
	sys.stdout.flush()
	asdf_fn = odir + f"MergerHistory_Final_z{snapin:4.3f}.{ii:03d}.asdf"
...
tend = time.time()
print(f"""
      Processing complete in {tend-tstart:4.2f}s!
      Total IO time: {io_time:4.2f}s
      Of which halo_info read time: {info_read_time:4.2f}s
      Total tree query time: {tree_time:4.2f}s
      Total sort time: {sort_time:4.2f}s
      Total match time: {match_time:4.2f}s
      Total temporary save time: {temp_save_time:4.2f}s
      Total temporary read time: {temp_read_time:4.2f}s
      Total fixing time: {total_fix_time:4.2f}s
      Total write time: {write_time:4.2f}s
      """)
