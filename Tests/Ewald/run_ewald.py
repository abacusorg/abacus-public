...
from Abacus.Tools import scatter_density
...
def run(
    ...
    sim_params = dict(
        CPD=cpd, Order=order, ForceOutputDebug=int(fod), StoreForces=int(not fod)
    )
...
def check_storeforces(param, dtype=DEFAULT_DTYPE, check=True, plot=True):
    ...
    NP = param['NP']
    rescale = 3.0 * (param['Omega_M'] - param.get('Omega_Smooth', 0.0)) / (8.0 * np.pi)

    acc = np.empty((NP, 3), dtype=dtype)
    i = 0
    for accfn in sorted(
        (Path(param.get('OutputDirectory')) / 'acc' / 'Step0001').glob('acc_*')
    ):
    ...
    read = Path(
        param.get('ReadStateDirectory', Path(param['WorkingDirectory']) / 'read')
    )
    auxfns = sorted(read.glob('aux*_*'))
    if not auxfns:
        # aux might be on another node and thus inaccessible, but maybe we did it all on this node
        auxfns = Path(param.get('LocalWorkingDirectory')).parent.glob(
            param['SimName'] + '*/read/aux*'
        )
    ...
    ref_acc = np.fromfile(
        Path(__file__).parent / 'reference_ewald_acc.double3', dtype=np.float64
    ).reshape(-1, 3)
...
def plot_storeforces(
    param, acc, ref_acc, pltfn='ewald_storeforces.png', show_perc=True
):
    ...
    def _plot(figsize, pltfn):
        ...
        print(
            f'Plotting to {pltfn} ...', flush=True
        )  # might take a few seconds to do the KDE
        ...
        scatter_density(ref_mag, frac_diff, ax, size=2.0, log=True, bw=0.01)
...
def make_ic_files(param, erase_ic=True):
    ...
    cpd = param['CPD']
    splits = np.linspace(-0.5, 0.5, cpd, endpoint=False)[1:]
    ...
    rvpid_dtype = np.dtype(
        [('pos', p.dtype, 3), ('vel', p.dtype, 3), ('pid', np.int64)], align=True
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-cpd', help='Cells-per dimension', type=int, default=DEFAULT_CPD
    )
    parser.add_argument(
        '-order', help='Multipole order', type=int, default=DEFAULT_ORDER
    )
    parser.add_argument(
        '-sweep', help='Run orders 2 through ORDER', action='store_true'
    )
    parser.add_argument(
        '-dtype', help='Float type', type=str, default='f4', choices=['f4', 'f8']
    )
    parser.add_argument(
        '-fod',
        help='Use ForceOutputDebug to get separate near and far forces',
        action='store_true',
    )
