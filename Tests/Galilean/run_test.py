...
aux12_t = np.dtype([('aux',np.uint64), ('aux2',np.uint32)], align=False)
...
def select_primaries(aux, slabstart, ci, state):
    ...
    primaries = np.empty(NP, dtype=aux12_t)

    i = 0
    for x in range(len(ci)):
        primary_ci = ci[x].reshape(cpd, -1)[:,ghost_radius:cpd-ghost_radius]
...
def main(nstep):
    abacus.run('abacus.par2', maxsteps=nstep, clean=True)
    ...
    primary_pid = primaries['aux'] & PID_MASK
    primary_pid.sort()
    ppd = int(round(state['ppd']))

    nbad = check_pid(primary_pid, ppd)
    if nbad == 0:
        print('All primary PIDs present with no duplicates.')
    else:
        raise ValueError(f'Test failed; found {nbad} bad PIDs')
