...
/*
  ...
  Adapted for use in Abacus under the MIT License.
*/
...
#define AVX512_MASK3_FNMA_ADD_FLOATS(X, Y, Z, MASK)               _mm512_mask3_fnmadd_ps(X, Y, Z, MASK) 
...
#define AVX512_RSQRT14_FLOAT(X)                                   _mm512_rsqrt14_ps(X)  // Approximate rsqrt: 2^14 = 6e-5 fractional error


#define AVX512_SVML_SQRT_FLOAT(X)                                 _mm512_svml_sqrt_ps(X)
#define AVX512_TRUNCATE_FLOAT_TO_INT(X)                           _mm512_cvttps_epi32(X)
#define AVX512_STORE_FLOATS_TO_MEMORY_UNALIGNED(X,Y)              _mm512_storeu_ps(X,Y)
...
#define AVX512_STORE_FLOATS_TO_MEMORY_UNALIGNED(X,Y)              _mm512_storeu_pd(X,Y)
