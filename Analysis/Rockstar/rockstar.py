...
def run_rockstar(slice_dirs, ncpu=1, minmembers=20, downsample=1, config_template_fn='abacus.cfg.template', SO=False):
    ...
    for slice_dir in slice_dirs:
        ...
        retcode = subprocess32.check_call(['./rockstar', '-c', 'abacus-auto.cfg', '-s', '0'])
        
        # Launch clients?
        
        # Run host-subhalo identifier?
        # Could we run this as a RUN_ON_SUCCESS?
        # find_parents looks broken.  Maybe another included util is more up-to-date?
        #if retcode == 0:
        #    with open('','r') as outfile:
        #        subprocess32.check_call('./util/find_parents', 'out_XYZ.list', '<box_size>', stdout=outfile, stderr=subprocess32.STDOUT)
